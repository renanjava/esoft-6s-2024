...
export class DecksService {
  ...
  async fetchCards(cardColor: Array<string>, quantityColor?: Array<number>) {
    if (quantityColor) {
      ...
      quantityColor.forEach(async e => {
        index++;
        console.log("dentro do quantityColor foreach: " + cardColor[index] + " " + quantityColor[index])
        console.log("retorno de uma fetchCards2: " + await this.fetchCards2(cardColor[index], quantityColor[index]))
      });
  ...
  async fetchCards2(cardColor: string, quantityColor?: number) {
    ...
    if (quantityColor) {
      console.log("quantidade do slice fetch2: " + cards.slice(0, quantityColor).length)
  ...
  async generateQuantityColors(colorQuantity, commander) {
    ...
    allowedColors.forEach(e => {
      ...
      const valueForSorting = Math.floor(this.sortedPossibilities(logicForSorting))
      console.log("e[0] foreach: " + e[0])
      console.log("variavel contadora foreach: " + index)
      console.log("cardQuantity: " + cardQuantity + "colorQuantity: " + colorQuantity)
      ...
      if (allowedColors[allowedColors.length - 1] != e)
        ...
        sortedNumber = 99 - cardQuantityColors.reduce((accumulator, currentValue) => {
  ...
  sortedPossibilities(value: number): number {
    const possibilities = new Array<number>(0.75, 0.50, 0.25)
